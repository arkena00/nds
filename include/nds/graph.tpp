...
namespace nds::internal
{
    ...
    ::add(T v)
    {
        ...
        node<T>* last_node = std::get<index>(nodes_).back().get();
    ...
    ::nodes(F&& f) const
    {
        ...
        {
            using input_node_type = node<typename std::decay_t<decltype(nt)>::type>;
    ...
    ::edges(F&& f) const
    {
        nds::cx::for_each<Edges>([&f, this](auto&& et)
        {
            ...
            using input_edge_type = nds::edge<nds::node<typename input_raw_edge_type::source_type>, nds::node<typename input_raw_edge_type::target_type>>;
    ...
    ::connect(int s, int t)
    {
        ...
        // static_assert(index >= 0, "connection between U and V not allowed");
        node<T>* source = std::get<0>(nodes_)[s].get();
        node<T>* target = std::get<0>(nodes_)[t].get();
